...
export const getAllArtikel = (req, res) => {
  ...
  db.query(sql, (err, results) => {
    if (err) {
      console.error(err);
      return res.status(500).json({ message: "Gagal ambil artikel" });
    }
...
export const getArtikelBySlug = (req, res) => {
  const { slug } = req.params;

  const sql = "SELECT * FROM articles WHERE slug = ? LIMIT 1";
...
export const updateArtikel = (req, res) => {
  const { id } = req.params;
  const { judul, slug, penulis, isi } = req.body;

  let thumbnailQuery = "";
  let params = [judul, slug, penulis, isi];
